...
// CurrencyTier + formatCurrencyLine + buildMsgEconomy extracted to
// desktop/src/lib/currencyDisplay.ts (SHA-FF.3) so they can be unit-tested in
// isolation. CollabTab consumers use the imported names directly.
import {
  formatCurrencyLine,
  buildMsgEconomy,
  type CurrencyTier,
...
export function CollabTab() {
  ...
  // SHA-CR.5 review-window state-aggregation logic extracted to
  // desktop/src/lib/reviewWindowAggregator.ts (SHA-CR.5.1) so the
  // aggregation is testable + reusable. CollabTab consumes via this
  // 3-line useMemo.
  const reviewWindowByCommit = useMemo(
    () => buildReviewWindowByCommit(project?.messages ?? []),
    [project?.messages],
  );
