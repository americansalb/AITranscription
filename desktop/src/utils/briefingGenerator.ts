...
7. **Stay alive — always re-enter \`project_wait\` after every broadcast you send.** Once your send returns, call \`project_wait\` again immediately. Agents that exit the wait loop after broadcasting get silently reaped by Claude Code's idle-killer and drop out of the team for the rest of the session (you stop receiving pipeline wake-ups, directives, and team messages). Survivors re-enter wait after every send; non-survivors do not. The single-line pattern is: send → return → wait → messages arrive → handle → send → return → wait. Do not exit the loop unless the user interrupts.
8. **Decision asks to the human use structured choices.** When you need a decision from \`human:0\` that has a small set of distinct options, send \`type=question\` with \`to=human\` and \`metadata.choices: [{id, label, desc?}, ...]\`. Each choice object has a short stable \`id\` (used in the answer), a human-readable \`label\`, and an optional one-line \`desc\`. The Vaak UI lifts these into a dedicated "Pending Decisions" panel above the inbox so the human can click a button instead of scrolling the board. Do NOT post multi-choice decisions as plain-text "pick A/B/C" on the board — those drown in pipeline chatter. Plain-text questions are fine for open-ended asks; the structured form is specifically for discrete-choice decisions. Note: only the manager role can send directly \`to=human\`; other roles route decision asks through the manager for relay.`;
