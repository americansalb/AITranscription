...
**NEW MW INSTANCE — watchdog regression from SHA-MW6.fix-2 (2026-05-28, architect:0 found at 15:04Z):** SHA-MW6.fix-2 made `last_alive_at_ms` correctly track sidecar liveness via project_wait. The watchdog at `main.rs:7404 + 7412` was implicitly designed AROUND the prior bug — both branches gate release on `!heartbeat_fresh`. Pre-fix, polling agents had stale heartbeat (the bug) → AND-condition met → release fired. Post-fix, polling agents ALWAYS have fresh heartbeat → AND-condition never triggers → `floor_stall` and `max_floor_exceeded` are both permanently disabled for any seat doing project_wait. Live evidence: evil-architect:0 held the mic 15 min with stale `last_working_at` (14:49Z) but fresh heartbeat (15:04Z); watchdog never released. Multi-writer-class: two subsystems (heartbeat tracking + mic-release watchdog) had different semantic expectations of the same field; fixing one broke the other. Architect-lane queued follow-up options: (a) drop heartbeat gate from floor_stall (kicks tool-call workers); (b) SEPARATE Signal-A RPC heartbeat for the gate (proper fix, needs infra); (c) max_floor_FORCE ignoring heartbeat at longer threshold (simplest stopgap). Recommend (a) or (b) for proper fix; (c) as immediate stopgap. Memory candidate: `feedback_watchdog_assumed_busy_seat_heartbeat_bug_as_signal`.
