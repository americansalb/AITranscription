"""Multi-LLM translation service.

Supports Claude, GPT, Groq (Llama), and Gemini as translation backends.
The prompt is tuned for professional interpretation — faithful, natural,
and preserving tone and register.
"""
...
def _check_providers():
    """Populate available providers based on configured API keys."""
    global AVAILABLE_PROVIDERS
    AVAILABLE_PROVIDERS = {}
    if config.ANTHROPIC_API_KEY:
        AVAILABLE_PROVIDERS["claude"] = config.ANTHROPIC_MODEL
    if config.OPENAI_API_KEY:
        AVAILABLE_PROVIDERS["gpt"] = config.OPENAI_MODEL
    if config.GROQ_API_KEY:
        AVAILABLE_PROVIDERS["groq"] = config.GROQ_LLAMA_MODEL
    if config.GOOGLE_API_KEY:
        AVAILABLE_PROVIDERS["gemini"] = config.GOOGLE_MODEL
...
async def translate(
    ...
    Returns dict with: translated_text, provider, model
    """
    if not text.strip():
...
def get_available_providers() -> list[dict]:
    ...
    return [
        {"id": pid, "model": model}
        for pid, model in AVAILABLE_PROVIDERS.items()
    ]
