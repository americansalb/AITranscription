...
while (`$true) {
    ...
    # ---- Global pause: .vaak/seats-paused (or allow-stop) halts relaunch ----
    # Honors the same human-ordered pause sentinel as the Stop hook (Layer 4)
    # and the supervisor (Layer 2), so a deliberate pause is never fought. Unlike
    # the per-seat drain above this does NOT exit the wrapper — it polls, so
    # removing the file auto-resumes this seat via --resume. (pause = temporary;
    # disband a single seat = stop-<role>-<instance>; the per-seat drain still
    # wins if it appears mid-pause.)
    ...
    if (`$seatData -ne `$null -and `$attempt -gt 1) {
        ...
        if (`$seatData.force_fresh -eq `$true) {
            # Layer-2 supervisor flagged a fatigue/wedge relaunch (force_fresh:true). Relaunch
            # FRESH, NOT --resume (which reloads the bloated context). Consumed+cleared by the
            # fresh-branch seat.json write below. (busy-aware FRESH-relaunch spec 2026-05-29.)
